...
### Phase 14 — Observability, containers, CI/CD, security, and operational hardening

Merged to `main` as PR #14 at commit `b73951c107fa02c9c6ccc366221b9aafb77d205d` from final validated PR head `5eae2668b1543c7fc845fd6470d08f3e35b6705d`. Final report-bearing PR-head run `34834619090` and independent merge-triggered `main` run `34834859774` both passed all five jobs: quality, integration/full regression, clean container build/operational smoke/containerized full suite/outage drill, dependency/secret security scanning, and dedicated installed-Tesseract OCR.

Phase 14 adds sanitized Langfuse-compatible query/evaluation traces over the existing serving boundary; application-scoped low-cardinality Prometheus metrics and provisioned Grafana dashboards; safe aggregate query-length/embedding-norm drift hooks with explicit insufficient-data semantics; exact-origin CORS configuration, bounded security-sensitive headers, response hardening, and hashed fixed-window API-key rate limiting; a non-root multi-stage Docker runtime and Tesseract-enabled test target; API/Qdrant/Redis/Prometheus/Grafana Compose; a deterministic PR-fast evaluation gate; a separate scheduled/manual alert-only evaluation workflow; `pip-audit`, `detect-secrets`, and Dependabot; and explicit Qdrant reindex-first plus evaluation-artifact recovery guidance.

Accepted implementation evidence: Ruff lint/format passed; strict mypy passed on **90 source files**; unit suite **181 passed**; ordinary integration **31 passed, 1 skipped**; host cumulative suite **212 passed, 1 skipped**; Tesseract-enabled container cumulative suite **213 passed**; dedicated installed-Tesseract OCR passed. The runtime non-root assertion reported **UID 10001**. The operational fixture proved authenticated cited query, successful bounded evaluation job, `/metrics`, Prometheus scrape health, Grafana health, and a real Qdrant outage producing HTTP 503 readiness with `rageval_dependency_ready{component="qdrant"} 0.0`. Security CI passed both Python dependency auditing and secret scanning.

Phase 14 evidence remains mechanics/operations evidence. The target roughly 12,000-document corpus and roughly 200-question reviewed evaluation set are absent; live OpenAI/Cohere/Langfuse/W&B validation and production load/SLO/cost measurement were not run. Drift status is a configured aggregate heuristic, not statistical significance. The rate limiter is per process, `/metrics` requires deployment-layer network restriction in production, evaluation job state remains in-process/non-durable, Python dependencies are bounded rather than fully hash-locked, and Qdrant recovery is reindex-first rather than a tested environment-specific snapshot restore. Detailed evidence is in `docs/phases/phase-14-report.md` and `docs/operations.md`.

### Phase 15 — Full-system validation, benchmarking, and release readiness

Merged to `main` as PR #23 at commit `984fc627da264ce27c3ce092906f938ce81c9db0` from final report-bearing PR head `10d352bff0cf0dd1b92afe8e8a55086fd61a39df`. Final PR-head run `34855291806` and independent merge-triggered `main` run `34855636932` both passed all six jobs: quality, integration/full regression, clean container acceptance, dependency/secret security scanning, dedicated installed-Tesseract OCR, and the Phase 15 release-validation artifact gate.

Phase 15 adds typed full-system validation/benchmark evidence contracts and a single adversarial acceptance runner over the existing canonical parse/clean/chunk → Qdrant/BM25 → RRF → rerank → grounded generation/citations → FastAPI/cache → evaluation → tracing/metrics path. The release gate covers financial/legal/research E2E queries, table-aware retrieval, vocabulary-mismatch query expansion, bounded two-hop retrieval, insufficient-context refusal, mocked provider retry/timeout, Self-RAG comparison, evaluation checkpoint resume, index rebuild, modest API concurrency, trace/metric redaction, cache invalidation after index identity change, and installed-Tesseract OCR. A separate reconciliation step persists the original five-configuration runs/checkpoints/reports and computes fixture-local per-domain composites without promoting the tiny fixture into a production recommendation.

Accepted implementation evidence: Ruff lint/format passed; strict mypy passed on **92 source files**; unit suite **184 passed**; ordinary integration **31 passed, 1 skipped**; host cumulative suite **215 passed, 1 skipped**; Tesseract-enabled container cumulative suite **216 passed**; dedicated installed-Tesseract OCR passed; runtime UID remained **10001**. Container acceptance passed the real Phase 15 validator and demonstrated both Qdrant outage degradation (`rageval_dependency_ready{component="qdrant"} 0.0`) and recovery (`1.0`). Security CI passed dependency and secret scanning.

The authoritative implementation artifact from run `34852249615` records exact source SHA `31cc3f8bbcceb07a38672d1f8bc197347f99ec5f`, 4 source documents, 6 canonical chunks, 15 passed deterministic/adversarial scenarios, and `release_gate_passed=true`. Its runner-local fixture benchmark observed cold API p95 `6.026 ms`, warm-cache API p95 `0.462 ms`, and a 12-request/concurrency-4 run at `187.14 requests/second` with p95 `58.05 ms`. Those measurements are local fixture observations only, not production SLO, capacity, or hosted-provider claims.

The available reviewed set contains **3 records**, not the target roughly 200. The Phase 15 support/reference-backed mechanics evaluator completed `0/3` hallucination flags and then resumed all three checkpoints; the inherited Phase 11 deterministic-rule fixture independently flags `1/3`. Neither result is a production hallucination-rate estimate. The five-configuration reconciliation reports fixture-local ties (financial: fixed-1024/semantic/table-aware; legal: fixed-1024/table-aware; research: fixed-1024/semantic/table-aware), so no production chunking winner is selected.

The representative roughly 12,000-document corpus and roughly 200-question reviewed evaluation set remain absent and were not fabricated. Live OpenAI/Cohere/Langfuse/W&B/RAGAS acceptance, production load/SLO/cost measurement, statistical drift significance, and a ColPali/vision-table adapter remain unvalidated or outside current roadmap scope. Detailed evidence and limitations are in `docs/phases/phase-15-report.md` and `docs/phases/phase-15-postmerge.md`.

## Next work

Post-v0.1.0 engineering remains production deployment/platform hardening, distributed rate limiting or durable job execution where required by topology, dependency locking/maintenance, production monitoring retention, real corpus/index bootstrap, real reviewed held-out evaluation, hosted-provider/live trace validation when credentials are deliberately supplied, representative load/quality/cost measurement, and statistically meaningful drift validation. These are explicit future evidence tiers rather than claims attached to the deterministic fixture release.
