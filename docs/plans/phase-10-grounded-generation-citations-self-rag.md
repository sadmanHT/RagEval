# Phase 10 — Grounded Generation, Context Assembly, Citations & Self-RAG Routing
...
### Mission
Generate answers that are explicitly grounded in retrieved context, can refuse insufficient evidence, and expose machine-verifiable citations.

### Required implementation
- Implement context assembler with deterministic chunk ordering, token-budget enforcement, chunk IDs, source metadata, and clear separators.
- Prevent duplicate/near-duplicate chunks from wasting context.
- Implement provider-abstracted LLM generation with at least the reference hosted path (OpenAI and/or Anthropic) and deterministic fake generation for tests.
- System behavior:
  - answer only from provided context;
  - cite supporting chunk IDs for factual claims;
  - explicitly say when context is insufficient;
  - do not silently use unsupported external knowledge.
- Define a structured `GroundedAnswer` output with answer text, citations, cited chunk IDs, provider/model metadata, token usage, latency, and refusal/insufficient-context status.
- Validate citations after generation: cited IDs must exist in supplied context.
- Add a bounded repair step for malformed structured output/citations; record repairs.
- Implement a simple Self-RAG-style routing layer if desired by roadmap: decide whether retrieval is necessary, but default to retrieval for domain questions and make the decision observable. Never let a “no retrieval” path create ungrounded answers while claiming the same faithfulness guarantees.
- Add prompt-injection resistance at the context boundary: retrieved document instructions are data, not system instructions.

### Testing required in this phase
- Context budget/truncation and deterministic ordering tests.
- Citation validation: nonexistent IDs fail or trigger bounded repair.
- Insufficient-context tests require explicit refusal.
- Prompt-injection fixture must not override system grounding rules.
- Fake-provider tests for success, malformed output, timeout, provider error, and repair limit.
- End-to-end local test from retrieval service to grounded answer.
- If live credentials exist, run a small live smoke but keep acceptance independent from it.
- Full regression suite across Phases 1–9.

### Phase completion gate
The project must produce a structured, cited, grounded answer from the fixture corpus and correctly refuse an unanswerable question, with all citations traceable to retrieved chunks.
