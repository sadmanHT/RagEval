# Phase 13 — FastAPI Serving, Authentication, Caching, Streaming & Evaluation Jobs
...
### Mission
Expose the system as a robust async service without weakening grounding, traceability, or testability.

### Required implementation
- Implement typed FastAPI application with versioned schemas.
- Required API capabilities:
  - `POST /query`: question + optional domain/filters/top_k/options; return grounded answer, citations, retrieval diagnostics according to exposure policy, and per-stage latency.
  - `GET /eval/latest`: return latest completed evaluation summary.
  - `POST /eval/run`: start an evaluation job and return a job/run ID.
  - add a job-status endpoint even if the reference document omitted it, because asynchronous run IDs require a supported polling contract.
  - health/readiness endpoints for process, Qdrant, Redis, and configured providers.
- Implement API-key authentication with constant-time comparison/secure handling and clear 401/403 semantics.
- Add request IDs, validation limits, request size constraints, timeouts, and safe error responses.
- Implement Redis caching only where correctness is clear. Cache keys must include query, retrieval filters, relevant configuration/index fingerprint, and model/prompt version so stale answers are not served after reindex/config changes.
- Implement optional streaming response mode while preserving final structured citations.
- Avoid blocking the event loop; isolate CPU-heavy or synchronous library calls.
- Evaluation job execution must have bounded concurrency and must not run uncontrolled in web request workers.

### Testing required in this phase
- API schema tests and OpenAPI generation.
- Auth success/failure/missing-key tests.
- Query endpoint integration through fake providers and real local Qdrant/Redis.
- Cache hit/miss/invalidation tests after index/config changes.
- Streaming test validates final citation payload and disconnect handling.
- Evaluation job create/status/latest lifecycle tests.
- Health/readiness degradation tests when Qdrant/Redis are unavailable.
- Concurrency test for multiple query requests.
- Security regression: secrets not returned in errors/logs.
- Full cumulative suite.

### Phase completion gate
A local client must be able to authenticate, query the fixture corpus, receive a grounded cited answer, trigger/check an evaluation job, and observe correct failure behavior when dependencies are unhealthy.
